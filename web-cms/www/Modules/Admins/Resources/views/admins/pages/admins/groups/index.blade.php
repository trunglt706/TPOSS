...
@section('content')
    <div class="content-wrapper">
                                    ...
                                    <form class="form-filter w-100">
                                        <div class="row">
                                            <div class="col-md-4 col-sm-6 mb-1">
                                                <label class="form-label">@lang('exist_admin')</label>
                                                <select name="has_admin" class="select2 form-select"
                                                    onchange="filterTable()">
                                                    <option value="">--- @lang('no_select') ---</option>
                                                    <option value="1">{{ __('exist_admin') }}</option>
                                                    <option value="2">{{ __('not_exist_admin') }}</option>
                                                </select>
                                            </div>
        ...
        {{-- create new data --}}
        @include('admins::admins.pages.admins.groups.create')
        {{-- End create new data --}}
    </div>
@endsection
...
@section('script')
    <script>
        function filterTable(page = 1) {
            ...
            load_ajax("{{ route('admin.admin_groups.list') }}?page=" + page + "&" + data, $('.table-responsive'), true);
        }
        ...
        $(".modalCreate form").on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.admin_groups.store') }}",
